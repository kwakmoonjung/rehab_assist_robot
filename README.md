...
> **Note:** 아래에 실제 알고리즘 순서도 이미지를 삽입해 주세요.
1. Start -> 안면 인식 (사용자 식별)
2. 운동 UI 선택 (사레레 / 숄더프레스)
3. YOLOv11 Pose 실시간 트래킹 시작
4. 자세 분석 알고리즘 (정상 범주 판별)
   - [자세 불량 시] 로봇 암 가이드 및 음성 피드백
   - [자세 정확도 충족 시] 운동 횟수 카운트 업데이트
5. 운동 종료 -> 로봇 `init_pos` 복귀 및 DB 저장
