...
class ExerciseSessionTracker:
    ...
    def reset(self):
        ...
        self.emit_data() # [수정] 초기화 시점에도 한 번 퍼블리시
    ...
    def _safe_avg(self, value_sum):
        if self.analyzed_frame_count == 0: return 0.0
        return round(value_sum / self.analyzed_frame_count, 2)

    def emit_data(self): # [수정] save -> emit_data 로 변경 및 파일 IO 제거
        good_ratio = 0.0
...
class BicepCurlAnalyzer:
    def __init__(self, publish_callback): # [수정] 콜백 전달받기
        self.count = 0
        self.tracker = ExerciseSessionTracker("bicep_curl", publish_callback)
        # ===== 자세 기준 (기존과 동일) =====
