...
def find_checkerboard_pose(
    ...
    # 수정 내역 1: 하드코딩되어 있던 25를 파라미터 square_size로 변경
    objp[:, :2] = (
...
# A와 B의 변환을 이용하여 보정 행렬 계산 (원본 로직 완벽 유지)
def Calibrate(A, B):
    n_data = len(A)
    M = np.zeros((3, 3))
...
if __name__ == "__main__":
    ...
    # 기존 기능: 직접 구현하신 Calibrate 함수 실행 결과
    ...
    print("\n[직접 구현한 캘리브레이션 결과]")
    print(T_cam2base_custom)
    print(T_cam2base_custom[:3, 3])
    np.save("T_cam2base.npy", T_cam2base_custom)

    # 추가 내역: 포럼에서 확인한 Eye-to-Hand 교차 검증 로직 추가 (OpenCV 내장 함수)
    # 기존 계산된 리스트를 활용하여 OpenCV 함수로 한 번 더 검증하는 안전장치 기능입니다.
    R_base2g_arr = np.array(R_gripper2base_list)
    t_base2g_arr = np.array(t_gripper2base_list)
    R_t2c_arr = np.array(R_checker2camera_list)
    t_t2c_arr = np.array(t_checker2camera_list)
    ...
    T_cam2base_cv[:3, :3] = R_cam2base_cv
    T_cam2base_cv[:3, 3] = t_cam2base_cv.flatten()

    print("\n[OpenCV 내장 함수 (Eye-to-Hand) 캘리브레이션 결과]")
    print(T_cam2base_cv)
    print(T_cam2base_cv[:3, 3])

    # --- 추가 및 수정된 저장 로직 ---
    # 1. Park & Martin (직접 구현) 결과 저장
    np.save("T_cam2base_park.npy", T_cam2base_custom)
    print("\n[저장 완료] T_cam2base_park.npy")

    # 2. OpenCV (Tsai-Lenz) 결과 저장
    np.save("T_cam2base_tsai.npy", T_cam2base_cv)
    print("[저장 완료] T_cam2base_tsai.npy")