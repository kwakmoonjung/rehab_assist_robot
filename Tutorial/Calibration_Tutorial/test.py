...
class TestNode(Node):
    def __init__(self):
        ...
        self.gripper2cam = np.load("T_cam2base_daniilidis.npy")
    ...
    def pick_and_drop(self, x, y, z):
        ...
        # [추가] 캘리브레이션 오프셋 적용 및 목표 좌표, 홈 좌표 변수 추가
        # pick_pos = posx([x, y, z+195, current_pos[3], current_pos[4], current_pos[5]])  # 195: 직접 캘리브레이션한 파일 기준
        pick_pos = posx([-x+1100, y+130, z+100, current_pos[3], current_pos[4], current_pos[5]])  # 195: 직접 캘리브레이션한 파일 기준
        home_pos = posj([0, 0, 90, 0, 90, 0])
        
        # [추가] 지정된 좌표로 직선 이동(movel) 추가
        movel(pick_pos, vel=50, acc=50)
        self.gripper.close_gripper()
        wait(1)

        # [추가] 초기 위치로 관절 이동(movej) 추가
        movej(home_pos, vel=50, acc=50)
